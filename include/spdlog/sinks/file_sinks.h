...
protected:
    void _sink_it(const details::log_msg& msg) override
    {
        ...
        if (_current_size > _max_size || std::chrono::system_clock::now() >= _rotation_tp)
        {
            _file_helper.open(calc_filename(_base_filename));
            _rotate();
            _rotation_tp = _next_rotation_tp();
...
private:
    static filename_t calc_filename(const filename_t& filename)
    {
        ...
        char mbstr[100] = {0};
        std::strftime(mbstr,100,"%Y-%m-%d-%H%M%S",tm_ptr);
        if (strlen(mbstr) != 0)
            w.write(SPDLOG_FILENAME_T("{}_{}_{}.log"), filename,details::os::pid(),mbstr);
        else
            w.write(SPDLOG_FILENAME_T("{}_{}.log"), filename,details::os::pid());
        return w.str();
    }
    ...
    void _rotate()
    {
        ...
        _file_helper.close();
        filename_t target = calc_filename(_base_filename);
